# Connector level
set conParams(InitDim)                          31; # Initial connector dimension
set conParams(MaxDim)                           35; # Maximum connector dimension
set conParams(MinDim)                           25; # Minimum connector dimension
set conParams(TurnAngle)                       0.000000; # Maximum turning angle on connectors for dimensioning (0 - not used)
...
set conParams(ProxGrowthRate)                  1.300000; # Connector proximity growth rate
...
set domParams(TRexARLimit)                     200.000000; # Domain T-Rex maximum aspect ratio limit (0 - not used)
...
set domParams(MaxEdge)                         0.150000; # Domain maximum edge length
set domParams(Adapt)                            0; # Set up all domains for adaptation (0 - not used) V18.2+ (experimental)
set domParams(WallSpacing)                     0.000000; # defined spacing when geometry attributed with $wall
...
set blkParams(collisionBuffer)                 0.500000; # Collision buffer for colliding T-Rex fronts
set blkParams(maxSkewAngle)                    180.000000; # Maximum skew angle for T-Rex extrusion
set blkParams(TRexSkewDelay)                    0; # Number of layers to delay enforcement of skew criteria
set blkParams(edgeMaxGrowthRate)               1.800000; # Volumetric edge ratio
set blkParams(fullLayers)                       1; # Full layers (0 for multi-normals, >= 1 for single normal)
set blkParams(maxLayers)                        7; # Maximum layers
set blkParams(growthRate)                      2.000000; # Growth rate for volume T-Rex extrusion
