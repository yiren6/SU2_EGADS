...
def find_boundary_index(su2_file_path):
...
def write_marker_sensitivity(sensitivity_data, marker_index, selected_marker, objective_name,
...
def read_objective_function_val(force_breakDown_file_path, objective_val_label):
    ObjectiveValue = []
...
def read_surface_sensitivity(surface_sens_file_name, selected_marker, is_euler=False):
    ...
    # * is_euler indicate if the reader shall treat surface sensitivity file as generated
    #       by Euler or by RANS modeling
        ...
        for l in lines[1:]:  # start extracting data from 2nd to end
            if len(l) <= 90:
                ...
                if is_euler == False:
                    ...
                    # euler data
                    sensitivity_x = temp_line_conv_vec[9]
                    sensitivity_y = temp_line_conv_vec[10]
                    sensitivity_z = temp_line_conv_vec[11]

            currentTuple = (sensitivity_x, sensitivity_y, sensitivity_z)
            surfaceSens = (*surfaceSens, currentTuple)
    surface_sens_dict = {selected_marker: list(surfaceSens)}
    return surface_sens_dict


def read_marker_index(surface_sens_file_name):
    # * surfaceSensitivityFileName is a string of
    #       the file name of adjoint objective surface sensitivity
    #       usually named as "SURFACE_ADJOINT.csv"
    # * function output a list of index corresponding to first colum of the csv
    marker_index = ()
    with open(surface_sens_file_name) as surface_adj:
        lines = surface_adj.readlines()
        for l in lines[1:]:  # start extracting data from 2nd to end
...
def multiple_elementwise(tuple1, tuple2, marker_index):
