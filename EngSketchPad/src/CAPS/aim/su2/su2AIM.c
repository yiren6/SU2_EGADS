                  ...
                  int *nFields, char ***fnames, int **franks, int **fInOut)
{
    ...
    *instStore = su2Instance;
    ...
    // Design information
    status = initiate_cfdDesignStruct(&su2Instance->design);
    AIM_STATUS(aimInfo, status);
    ...
    if (index == Proj_Name) {
      ...
      if (su2Instance != NULL) su2Instance->su2Version = defval;
        ...
        *ainame              = EG_strdup("Design_Variable");
         ...
         * List of GeometryIn variable names used to compute sensitivities of Design_Functional for optimization, see \ref cfdDesignVariable for additional details.
         */
        ...
        *ainame              = EG_strdup("Design_Sensitivity");
         ...
         * If True and Design_Functional is set, create geometric sensitivities SU2 input files needed to compute Design_Functional sensitivities w.r.t Design_Variable.
                   ...
                   capsValue *aimInputs)
{
    ...
    if (aimInputs[Design_Variable-1].nullVal == NotNull &&
        (su2Instance->design.numDesignVariable == 0 ||
...
/*@-nullpass@*/
        status = cfd_getDesignVariable(aimInfo,
                                       ...
                                       &su2Instance->design.numDesignVariable,
                                       &su2Instance->design.designVariable);
/*@+nullpass@*/
        AIM_STATUS(aimInfo, status);
    }
    ...
    if ( aimInputs[Design_Functional-1].nullVal == NotNull &&
        (su2Instance->design.numDesignFunctional == 0 ||
        ...
        status = cfd_getDesignFunctional(aimInfo,
                                         ...
                                         su2Instance->design.numDesignVariable,
                                         su2Instance->design.designVariable,
                                         &su2Instance->design.numDesignFunctional,
                                         &su2Instance->design.designFunctional);
        AIM_STATUS(aimInfo, status);
    }
    ...
    if (usePython == (int) true && pythonLinked == (int) true) {
        ...
        if (aimInputs[Overwrite_CFG-1].vals.integer == (int) false) {
            ...
            if (strcasecmp(aimInputs[SU2_Version-1].vals.string, "Cardinal") == 0) {
                ...
                status = su2_writeCongfig_Blackbird(aimInfo, aimInputs, meshfilename, su2Instance->bcProps, su2Instance->design, withMotion);
                    ...
                    /*@unused@*/ int restart, /*@unused@*/ capsValue *aimInputs)
{
  ...
  if (aimInputs[Design_Sensitivity-1].vals.integer == (int)true) {
    ...
    nGeomIn = 0;
    for (i = 0; i < su2Instance->design.numDesignVariable; i++) {

      name = su2Instance->design.designVariable[i].name;
    ...
    // create a map from the volume vertices to the surface tessellation vertices
    // 1-based indexing means numVolNode will be the count
    numVolNode = 0;
    for (ibody = 0; ibody < meshRef->nmap; ibody++) {
      ...
      for (i = 0; i < offset; i++) {
        numVolNode = MAX(meshRef->maps[ibody].map[i], numVolNode);
      }
    }
    ...
    // Read <Proj_Name>.sens
    snprintf(tmp, 128, "%s%s", su2Instance->projectName, ".sens");
    ...
    if (su2Instance->design.numDesignVariable != numDesignVariable+nGeomIn) {
      AIM_ERROR(aimInfo, "Incorrect number of design variables in sens file. Expected %d and found %d",
                su2Instance->design.numDesignVariable-nGeomIn, numDesignVariable);
      status = CAPS_IOERR; goto cleanup;
    }
    ...
    for (i = 0; i < numFunctional; i++) {
      ...
      AIM_ALLOC(values[i].derivs, su2Instance->design.numDesignVariable, capsDeriv, aimInfo, status);
      for (idv = 0; idv < su2Instance->design.numDesignVariable; idv++) {
        values[i].derivs[idv].name  = NULL;
        values[i].derivs[idv].deriv = NULL;
        values[i].derivs[idv].len_wrt = 0;
      }
      values[i].nderiv = su2Instance->design.numDesignVariable;
    }
    ...
    for (i = 0; i < numFunctional; i++) {
      ...
      for (k = nGeomIn; k < su2Instance->design.numDesignVariable; k++) {
        ...
        found = (int)false;
        for (idv = 0; idv < su2Instance->design.numDesignVariable; idv++)
          if ( strcasecmp(su2Instance->design.designVariable[idv].name, tmp) == 0) {
            found = (int)true;
            break;
          }
        if (found == (int)false) {
    ...
    for (idv = 0; idv < su2Instance->design.numDesignVariable; idv++) {
      ...
      name = su2Instance->design.designVariable[idv].name;
